...
# Environment variable to control SSL verification for debugging
...
# Updated formatter to include module and line number for better context
...
# Configure Gunicorn loggers to use the same handlers and level
...
gunicorn_access_logger.handlers = root_logger.handlers # Using root_logger handlers for access logs too
gunicorn_access_logger.setLevel(effective_log_level) # Or set to INFO if DEBUG is too verbose for access
...
def before_request_logging_extended():
    ...
    g.request_id = str(uuid.uuid4()) # Assign a unique ID to each request
    app.logger.debug(f"RID-{g.request_id}: Request received: {request.method} {request.path} from {request.remote_addr}")
    if app.logger.isEnabledFor(logging.DEBUG): # Only log headers and body if DEBUG is enabled
        headers_dict = dict(request.headers)
        if "Authorization" in headers_dict: # Mask sensitive Authorization header
        ...
        if request.data:
            try:
                # Try to decode as text, limit size for logging
...
def after_request_logging_extended(response):
    ...
    request_id = getattr(g, 'request_id', 'NO_REQUEST_ID_IN_G_AFTER') # Retrieve request ID from Flask's g
    # Log response status and a sample of the response data
...
def get_kfp_client():
    ...
    request_id_str = "KFP_CLIENT_INIT_NO_CTX" # Default request ID if not in Flask context
    if has_request_context() and hasattr(g, 'request_id'):
        request_id_str = g.request_id
    
    current_app_logger = app.logger # Use Flask app's logger
    ...
    token = None
    # Attempt to read the ServiceAccount token if the path exists
    if os.path.exists(KFP_SA_TOKEN_PATH):
        ...
        current_app_logger.info(f"RID-{request_id_str}: SA token path {KFP_SA_TOKEN_PATH} not found. KFP client will rely on ambient credentials if any.")

    try:
        # For KFP SDK v2.x, we configure the default kfp_server_api.Configuration
        # This default configuration will be used by KFPClient() if no specific config is passed to its constructor.
        default_kfp_config = KFPConfiguration.get_default_copy()
        default_kfp_config.host = KFP_ENDPOINT # Set the KFP API host

        # SSL/TLS Configuration
        if KFP_ENDPOINT.startswith('https://'):
            if KFP_VERIFY_SSL:
                # If KFP_VERIFY_SSL is true, try to use REQUESTS_CA_BUNDLE if set,
                # otherwise default to the standard in-pod service CA bundle.
                ...
                if os.path.exists(service_ca_path):
                    ...
                    # Rely on system's default CAs if specified path doesn't exist
                    default_kfp_config.verify_ssl = True 
            else: # KFP_VERIFY_SSL is False
        ...
        # Authentication: Set up Bearer token if available
        if token:
        ...
        # Apply the modified configuration as the default for kfp_server_api
        KFPConfiguration.set_default(default_kfp_config)
        
        # KFPClient() will use the default configuration set above.
        # The `existing_token` parameter in KFPClient constructor is more of a KFP SDK v1 style.
        # For v2, configuring the default KFPConfiguration is the preferred way.
        ...
        try:
            ...
            # KFP SDK v2 list_experiments returns a V2beta1ListExperimentsResponse object
        ...
        except kfp_server_api.ApiException as e_kfp_api: # kfp_server_api is still used for exceptions
            current_app_logger.error(f"RID-{request_id_str}: KFP API call (list_experiments) failed: Status {e_kfp_api.status}, Reason: {e_kfp_api.reason}, Body: {e_kfp_api.body}", exc_info=False) # Set exc_info=False for brevity in this specific case
            ...
            if not isinstance(verify_err, kfp_server_api.ApiException): # Log full trace for non-KFP API exceptions
...
def healthz():
    ...
    request_id = getattr(g, 'request_id', 'HEALTHZ_NO_G_ROUTE') # Get request ID if available
...
def handle_s3_event():
    ...
    request_id = getattr(g, 'request_id', "S3_EVENT_NO_G_ROUTE") # Get request ID
    ...
    cloud_event_id_received = cloudevent_headers.get("ce-id", "N/A") # Get CloudEvent ID from headers

    try:
        # Determine event format (Structured CloudEvent, Binary CloudEvent, or raw JSON)
        if "application/cloudevents+json" in str(request.content_type).lower():
            ...
            if isinstance(full_payload, dict) and 'id' in full_payload: # Get CloudEvent ID from payload if structured
                 cloud_event_id_received = full_payload.get('id')
        elif cloudevent_headers.get("ce-specversion") and request.data: # Check for binary CloudEvent
        ...
        elif request.is_json: # Fallback for plain JSON
        ...
        elif request.data : # Fallback for non-JSON data, try to parse as JSON
            try:
                minio_event_data = json.loads(request.data.decode('utf-8'))
        ...
        else: # No data or unsupported Content-Type
        ...
        # Validate extracted MinIO event data
        ...
        # Extract S3 bucket and object key from the event data
        bucket_name = "unknown_bucket"
        object_key = "unknown_key"
        eventName = minio_event_data.get('EventName') # MinIO specific top-level field

        # MinIO events often have a 'Records' array, similar to AWS S3 events
        if 'Records' in minio_event_data and isinstance(minio_event_data.get('Records'), list) and len(minio_event_data['Records']) > 0:
            ...
            if not eventName and 'eventName' in record: # EventName might be inside the record
        ...
        # Fallback for simpler MinIO event structures (e.g., from mc webhook)
        elif 'Key' in minio_event_data: 
             object_key = minio_event_data.get('Key', object_key)
             # Bucket name might be nested differently
        ...
        # Check if the object is a PDF (simple check, adjust as needed)
        is_pdf = False
        ...
        # For debugging, proceed even if not a PDF. Add filtering logic here if needed.
        ...
        # Get KFP client
        if not KFP_ENDPOINT: # Should have been checked by get_kfp_client, but double-check
        ...
        pipeline_id = None
        try:
            # For KFP SDK v2, list_pipelines and filter by name
        ...
        # Get or create KFP Experiment
        experiment = None
        try:
            # KFP SDK v2 uses experiment_name directly in create_run_from_pipeline_id
            # but we can still get/create the experiment for organization or if an ID is needed
            ...
            if experiments_response and experiments_response.experiments:
                ...
                experiment = kfp_client.create_experiment(experiment_name=KFP_EXPERIMENT_NAME) # KFP SDK v2
            ...
            # Proceeding without experiment_id if creation fails, as create_run_from_pipeline_id might work with experiment_name
        ...
        # Trigger the KFP pipeline run using KFP SDK v2
        run_result = kfp_client.create_run_from_pipeline_id(
            pipeline_id=pipeline_id,
            arguments=pipeline_params,
            run_name=run_name,
            experiment_name=KFP_EXPERIMENT_NAME # KFP SDK v2 often uses experiment_name
            # If your KFP backend specifically requires experiment_id:
            # experiment_id=experiment.experiment_id if experiment else None, 
        )
        
        run_id_str = getattr(run_result, 'run_id', "N/A") # KFP SDK v2 uses 'run_id'
        run_name_str = getattr(run_result, 'name', run_name) # KFP SDK v2 uses 'name'
        # Constructing the URL might need adjustment based on your KFP UI structure for v2
...
if __name__ == '__main__':
    # This section is for direct Flask execution (e.g., local testing), not for Gunicorn.
    port = int(os.environ.get("PORT", 8080))
    # FLASK_DEBUG is useful for local development, should be false in production.
