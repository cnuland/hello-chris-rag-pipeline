...
** *What it is:* Provides external knowledge to a generic model at the time of the query.
** *How it works:* The system retrieves relevant documents from a database and provides them as context alongside the user's prompt.
** *Analogy:* Giving a smart person an open book during an exam. They can find the answer, but their fundamental knowledge doesn't change.
** *Strengths:* Excellent for rapidly changing information and providing answers based on specific, verifiable documents.
** *Weakness:* Does not improve the model's core reasoning, style, or understanding of a specialized domain.

* **Retrieval-Augmented Fine-Tuning (RAFT):**
** *What it is:* Uses your curated, high-quality knowledge to *actually teach the model*, fundamentally improving its understanding.
** *How it works:* Your domain-specific data is used to generate a large set of training examples, which are then used to fine-tune the base LLM.
** *Analogy:* Having the smart person study the book *before* the exam. They internalize the knowledge and can answer questions more naturally and accurately.
** *Strengths:* Improves the model's core reasoning, its ability to adopt a specific persona (e.g., a helpful IT agent), and its accuracy within your domain.
** *Weakness:* The "baked-in" knowledge is only as current as the last fine-tuning run.
