= 3.5 Creating and Configuring a Pipeline Run
...
2.  This will open the "Create run" page. You can give the run a name and description, but the most important part is the **Pipeline parameters** section at the bottom.
...
3.  You must fill in the following parameters to tell the pipeline how to connect to the other services running in your project. Copy the values below exactly.
    ...
    * `api_endpoint`: The internal URL of the mock API you deployed.
    ...
    [source,text]
    ----
    http://mock-api:8080/api/v1/incidents
    ----
...
5.  Click the **Start** button to launch the pipeline run.
